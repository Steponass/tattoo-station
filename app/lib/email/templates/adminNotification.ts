...
function buildCustomerSection(input: AdminNotificationInput): string {
  return `
    <tr>
...
function buildServiceSection(input: AdminNotificationInput): string {
  ...
  return `
    <tr>
      <td style="padding: 0 0 24px 0;">
        <p style="margin: 0 0 8px; font-size: 13px; font-weight: bold; color: #B01E28; text-transform: uppercase; letter-spacing: 0.08em;">Service</p>
        <table cellpadding="0" cellspacing="0" style="width: 100%; border-left: 2px solid #B01E28; padding-left: 12px;">
          <tbody>
            ${buildDetailRow("Category", categoryLabel)}
            ${buildDetailRow("Type", serviceTypeLabel)}
            ${buildDetailRow("Artist", input.artistName ?? "No preference")}
...
export function buildAdminNotificationEmail(
  ...
  const { reference, submission, photoUrls } = input;

  const subject = `[${reference}] New booking request — ${input.customerName}`;
          ...
          <tr>
            <td style="background-color: #111111; padding: 16px 32px; text-align: center;">
              <span style="color: #888888; font-size: 12px;">
                tattoostation.lt · admin notification
              </span>
            </td>
          </tr>
