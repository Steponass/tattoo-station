...
const config: IntlayerConfig = {
  internationalization: {
    locales: [Locales.ENGLISH, Locales.LITHUANIAN],
    /**
     * Default locale used as a fallback if the requested locale is not found.
     */
    defaultLocale: Locales.LITHUANIAN,
  },
  routing: {
    /**
  ...
  compiler: {
    /**
     * Disabled deliberately. When enabled, the compiler auto-extracts every
     * string literal it finds into generated `.content.ts` dictionaries and
     * rewrites the source files in place — including SQL queries, console
     * logs, internal error details and CSS selectors, none of which are
     * user-facing copy. It also rewrites files while `npm run dev` is
     * running, so hand-fixing the damage is impossible until this is off.
     *
     * Dictionaries in this project are hand-authored in `*.content.ts` next
     * to the component that consumes them. Turn this back on only for a
     * one-off, reviewed extraction run.
     */
    enabled: false,
